...
import (
	...
	"github.com/MonCatCat/humans/testutil/network"
)
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices,
		fmt.Sprintf("0.000006%s",
			humans.AttoHeart),
		"Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01heart,0.001stake)")
...
Example:
	humansd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "humansd", "Home directory of the node's daemon configuration")
...
Example:
	humansd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("humans_%d-1", tmrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, humans.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(humans.AttoHeart, accStakingTokens),
		}

		genBalances = append(genBalances, banktypes.Balance{Address: addr.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, &humans.EthAccount{
			BaseAccount: authtypes.NewBaseAccount(addr, nil, 0, 0),
			CodeHash:    common.BytesToHash(evmtypes.EmptyCodeHash).Hex(),
		})

		valTokens := sdk.TokensFromConsensusPower(100, humans.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(humans.AttoHeart, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(humans.AttoHeart)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, humans.AttoHeart, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
