...
import (
	...
	humansclient "github.com/MonCatCat/humans/client"
	"github.com/MonCatCat/humans/client/debug"
	"github.com/MonCatCat/humans/crypto/hd"
	"github.com/MonCatCat/humans/encoding"
	"github.com/MonCatCat/humans/ethereum/eip712"
	"github.com/MonCatCat/humans/server"
	servercfg "github.com/MonCatCat/humans/server/config"
	srvflags "github.com/MonCatCat/humans/server/flags"
	humans "github.com/MonCatCat/humans/types"
)

const EnvPrefix = "HUMANS"
...
func NewRootCmd() (*cobra.Command, params.EncodingConfig) {
	...
	rootCmd.AddCommand(
		...
		humansclient.NewTestnetCmd(app.ModuleBasics, banktypes.GenesisBalancesIterator{}),
		debug.Cmd(),
		config.Cmd(),
	)
	...
	rootCmd.AddCommand(
		...
		queryCommand(),
		txCommand(),
		humansclient.KeyCommands(app.DefaultNodeHome),
	)
...
func (a appCreator) appExport(
	...
	if height != -1 {
		humansApp = app.NewHumansApp(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), a.encCfg, appOpts)

		if err := humansApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		humansApp = app.NewHumansApp(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), a.encCfg, appOpts)
	}

	return humansApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
